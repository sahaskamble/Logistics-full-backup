...
export default function ServiceRequestViewDetailsPage() {
  ...
  const { data: requests, isLoading } = useCollection('3pl_service_requests', {
    expand: 'order,order.provider,serviceType,service,merchantVerifiedBy,golVerifiedBy,user',
    filter: `id="${requestId}"`,
  });
  ...
  return (
    <div className="max-w-container mx-auto px-6 py-10 space-y-8 bg-[color:var(--accent)] rounded-lg shadow-lg">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <DetailCard label="Status" value={request.status} status color="primary" full />
        <DetailCard label="Service" value={request.expand?.service?.title || 'N/A'} />
      ...
      {/* provider Info */}
      {request.expand?.order?.expand?.provider && (
        <section>
          <h2 className="text-2xl font-semibold text-[color:var(--foreground)] mb-4">Provider Info</h2>
          <div className="bg-[color:var(--background-2)] p-5 rounded-md border border-[color:var(--primary)] space-y-2">
            <DetailCard label="Title" value={request.expand.order.expand.provider.title} />
            <DetailCard label="Location" value={request.expand.order.expand.provider.location} />
            <DetailCard label="Contact" value={request.expand.order.expand.provider.contact} />
          </div>
        </section>
      )}
